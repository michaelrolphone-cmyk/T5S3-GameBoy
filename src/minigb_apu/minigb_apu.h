...
/* The audio output format is in platform native endian. */
    ...
    !defined(MINIGB_APU_AUDIO_FORMAT_S32SYS)
...
/* One extra pair is needed on fractional host frames. */
...
#define AUDIO_SAMPLES_TOTAL_MAX	(AUDIO_SAMPLES_MAX * AUDIO_CHANNELS)
...
/**
 * Fill "stream" with sample_count stereo pairs and advance the APU by the
 * same amount. The caller must provide room for sample_count * AUDIO_CHANNELS
 * samples.
 */
void minigb_apu_audio_callback_samples(struct minigb_apu_ctx *ctx,
		audio_sample_t *stream, uint16_t sample_count);
