...
static IRAM_ATTR void update_square(struct minigb_apu_ctx *ctx, audio_sample_t *samples,
		const uint_fast16_t sample_count, const bool ch2)
{
	...
	for (uint_fast16_t i = 0; i < sample_count; ++i) {
		...
		samples[i * 2 + 0] += sample * c->on_left * ctx->vol_l;
		samples[i * 2 + 1] += sample * c->on_right * ctx->vol_r;
	}
}
...
static IRAM_ATTR void update_wave(struct minigb_apu_ctx *ctx, audio_sample_t *samples,
		const uint_fast16_t sample_count)
{
...
{
	minigb_apu_audio_callback_samples(ctx, stream, AUDIO_SAMPLES);
}

void IRAM_ATTR minigb_apu_audio_callback_samples(struct minigb_apu_ctx *ctx,
		audio_sample_t *stream, const uint16_t sample_count)
{
	memset(stream, 0,
		(size_t)sample_count * AUDIO_CHANNELS * sizeof(audio_sample_t));
	update_square(ctx, stream, sample_count, 0);
	update_square(ctx, stream, sample_count, 1);
	update_wave(ctx, stream, sample_count);
	update_noise(ctx, stream, sample_count);
}
...
{
	/* Initialise the full context so reset is independent of the prior ROM. */
	memset(ctx, 0, sizeof(*ctx));
	ctx->chans[0].val = ctx->chans[1].val = -1;
	/* Register writes below are ignored while NR52 reports power off. */
